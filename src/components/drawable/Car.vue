<template>
    <g>
        <defs>
            <svg id="car" viewBox="0 0 400.066 400.066">
                <path d="M319.776,175.379c-1,4.116-4.683,6.876-8.738,6.876c-0.704,0-1.418-0.083-2.133-0.256
        ...
        <!-- 
        <circle r="1.5" :cx="corners[0][0]" :cy="corners[0][1]" fill="blue"/>
        <circle r="1.5" :cx="corners[1][0]" :cy="corners[1][1]" fill="blue"/>
        <circle r="1.5" :cx="corners[2][0]" :cy="corners[2][1]" fill="red"/>
        <circle r="1.5" :cx="corners[3][0]" :cy="corners[3][1]" fill="red"/>

        <circle r="2" :cx="x" :cy="y" fill="green"/> 
        -->

        <text :x="x" :y="y" fill="red">{{id}}</text>
...
import getCorners from '../../lib/getCorners'

export default {
    props:['x','y','angle','w','h','id'],
    data:()=>({
    ...
    computed:{
        transform(){
            ...
            let tx = -this.w/2
            let ty = -this.h/2
            return `translate(${tx} ${ty}) rotate(${this.angle} ${cx} ${cy})`
        }
    },
    // watch:{
    //     $props:{
    //         handler(){
    //             this.corners = getCorners(this.x,this.y,this.w,this.h,this.angle)
    //         },
    //         deep:true
    //     }
    // }
